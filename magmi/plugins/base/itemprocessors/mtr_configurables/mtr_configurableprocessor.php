...
class Mtr_ConfigurableItemProcessor extends Magmi_ItemProcessor
{
...
/*
    public function afterImport(){
        $this->log('process configurables');
        $this->processConfigurables();
        $this->log('end import');
        return true;
    }
*/
	...
	{
		return array(
            "name" => "Mtr Configurable Item processor",
            "author" => "Victor Galitsky",
            "version" => "0.0.1",
			"url"=> $this->pluginDocUrl("Mtr_Configurable_Item_processor")
            );
	}
...
public function getConfigurableOptsFromAsId($asid)
{
	...
	{
        //collect simples
        if (isset($item['simple_no'])){
            if (!isset( $this->_items[$item['simple_no']] ) ){
                $this->_items[$item['simple_no']] = array();
            }
            $this->_items[$item['simple_no']][] = $item;
        }
        //unset($item['simple_no']);
        //return true;
    ...
    public function processConfigurables(){
        $this->log('factory');
        $dp=Magmi_DataPumpFactory::getDataPumpInstance("productimport");
        $dp->beginImportSession("default","create",new TestLogger());
        $this->log('factory end');

        foreach( self::_items as $simple_no => $simpleItems ){
            if ( count($simpleItems)<=1 )continue;
            $item=array(
                "store"=>"admin",
                "type"=>"configurable",
                "sku"=>$simpleItems[0]['sku'].'-C',
                "name"=>$simpleItems['name'],
                "description"=>$simpleItems[0]['description'] ? $simpleItems[0]['description'] : $simpleItems[0]['short_description'],
                "price"=>$simpleItems[0]['price'],
            );

            foreach( $simpleItems as $simleItem ){
                $subskus[] = $simleItem['sku'];
            }

            $item["configurable_attributes"]="color,size";
            $item["simples_skus"]=implode(",",$subskus);
            $this->log('ingest');
            $dp->ingest($item);
            $this->log('after ingest');
        }
        $dp->endImportSession();
        return true;
    }
    ...
    public function getMtrFixedSkus($_simple_no){
        $subskus = array();
        $items = $this->_items;
        $sitems = $items[$_simple_no];
        foreach( $sitems as  $simpleItem ){
                $subskus[] = $simpleItem['sku'];
        }
        return $subskus;
    }
...
$this->log(self::_items);
        $matchmode = 'mtr_fixed';
		switch($matchmode)
		{
            case "mtr_fixed":
                $sksus = $this->getMtrFixedSkus( $item['simple_no'] );
                $sskus=explode(",",$sksus);
                trimarray($sskus);
